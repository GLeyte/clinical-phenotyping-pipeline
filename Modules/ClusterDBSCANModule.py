...
This module provides DBSCAN (Density-Based Spatial Clustering of Applications with Noise)
functionality with:
- Configurable epsilon and min_samples parameters
...
_DEFAULT_PARAMETERS: Dict[str, Any] = {
    "eps": {"min": 1e-3, "max": 10.0, "log": True},
    "min_samples": {"min": 2, "max": 50},
}

_PENALTY_CLUSTERING_FAILED = -1.0
_PENALTY_CLUSTER_COUNT = -1.0
_PENALTY_MAX_SIZE = -1.0
...
# ============================================================================
# DBSCAN Cluster Helper
# ============================================================================


class DBSCANClusterHelper(ClusterBaseHelper):
    """
    DBSCAN clustering helper with Optuna-based hyperparameter optimisation.

    Extends ClusterBaseHelper with DBSCAN-specific functionality including:
    - DBSCAN clustering with configurable epsilon and min_samples
    - Metric evaluation over different parameter ranges
    - Optuna-based hyperparameter search
    ...
    Examples
    --------
    >>> helper = DBSCANClusterHelper(data=df, scaler='standard')
    >>> helper.clustering(eps=0.5, min_samples=5)
    ...
    def __init__(
        ...
        """
        Initialises DBSCANClusterHelper.
        ...
        logger.info(
            f"DBSCANClusterHelper initialized: {len(data)} records, scaler='{scaler}'"
        )
    ...
    def clustering(
        ...
        """
        Performs DBSCAN clustering and assigns cluster labels.
        ...
        eps : float, default=0.5
            ...
            in the neighbourhood of the other.
        ...
        logger.info(f"DBSCAN clustering complete: eps={eps}, min_samples={min_samples}")
    ...
    def plot_metrics(
        ...
        metrics_info : Dict[str, Any]
            ...
            where "config" contains tuples (eps, min_samples).
        ...
        x_labels = [
            f"eps={cfg[0]:.3f}\nmin_samples={cfg[1]}" for cfg in metrics_info["config"]
        ]
        ...
        for idx, metric in enumerate(metrics):
            ...
            config_text = f"eps={best_config[0]:.3f}\nmin_samples={best_config[1]}"
            ax.annotate(
                f"{config_text}\n{best_score:.4f}",
        ...
        plt.suptitle(
            "DBSCAN Clustering Metrics vs. Configuration",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def optuna_grid_search(
        ...
        """
        Optimises DBSCAN hyperparameters using Optuna.

        Searches over epsilon and min_samples with configurable cluster size constraints.
        ...
        parameters : Optional[Dict[str, Any]], default=None
            ...
            - 'eps': dict with 'min', 'max', optional 'log' (for log-scale sampling)
        ...
        def objective(trial: "optuna.Trial") -> float:
            """Optuna objective function for DBSCAN hyperparameter search."""
            eps = trial.suggest_float(
                "eps",
                parameters["eps"]["min"],
                parameters["eps"]["max"],
                log=parameters["eps"].get("log", True),
            )
        ...
        if save_storage:
            ...
            counter = 0
            while True:
                db_filename = f"dbscan-{suffix}-{metric}-{counter}.sqlite3"
            ...
            study_name = (
                f"Optuna Study DBSCAN {suffix.capitalize()} {metric.upper()} {counter}"
            )
    ...
    def plot_optuna_dashboard(
        ...
        optuna_dir = os.path.abspath("optuna-folder")
        db_filename = f"dbscan-{suffix}-{metric}-{study_index}.sqlite3"
        db_path = os.path.join(optuna_dir, db_filename)
...
if __name__ == "__main__":
    logger.info("DBSCANClusterHelper module loaded successfully")
